...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        polygon = isinstance(self, Polygon_Model)    # POLYGON
        ...
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=[ch], polygon=polygon)  # model, savelist
        self.names = [str(i) for i in range(self.yaml['nc'])]  # default names
        self.inplace = self.yaml.get('inplace', True)
        # logger.info([x.shape for x in self.forward(torch.zeros(1, ch, 64, 64))])

        # Build strides, anchors
        m = self.model[-1]  # Detect() or Polygon_Detect()
...
def parse_model(d, ch, polygon=False):  # model_dict, input_channels(3)
    polygon = polygon or ('polygon' in d['head'][-1][2].lower())    # POLYGON
    ...
    no = (na * (nc + 5)) if not polygon else (na * (nc + 9))    # POLYGON: (na*(nc + 9))
...
class Polygon_Detect(Detect):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            x[i] = x[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
        #     x[i] = x[i].view(bs * self.na * ny * nx, self.no).contiguous()#转换开，训练关
        # return torch.cat(x)
