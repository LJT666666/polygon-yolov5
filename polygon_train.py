...
from utils.loss import ComputeLoss, Polygon_ComputeLoss

from utils.plots import plot_images, plot_labels, plot_results, plot_evolution, polygon_plot_images, polygon_plot_labels
...
def train(hyp, opt, device, tb_writer=None, polygon=False):
    ...
    last = wdir / 'polygon_last.pt'
    best = wdir / 'polygon_best.pt'
    ...
    # polygon datasets start with "polygon": "polygon_ucas.yaml", "polygon_coco.yaml"
    ...
    if rank in [-1, 0]:
        testloader = create_dataloader(test_path, imgsz_test, batch_size * 2, gs, single_cls,
                                       ...
                                       world_size=opt.world_size, workers=opt.workers,
                                       pad=0.5, prefix=colorstr('val: '), polygon=polygon)[0]
...
#             model._initialize_biases(cf.to(device))
            ...
            # Anchors
            check_anchors = check_anchors if not polygon else polygon_check_anchors
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if rank in [-1, 0]:
                ...
                if plots and ni < 3:
                    ...
                    Thread(target=polygon_plot_images, args=(imgs, targets, paths, f), daemon=True).start()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='polygon_yolov5s_armor.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='data/polygon_armor.yaml', help='dataset.yaml path')
    parser.add_argument('--hyp', type=str, default='data/hyp.armor.yaml', help='hyperparameters path')
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(300):  # generations to evolve
            ...
            # Train mutation
            results = train(hyp.copy(), opt, device, polygon=opt.polygon)
